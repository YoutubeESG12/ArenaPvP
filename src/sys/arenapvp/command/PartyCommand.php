...
class PartyCommand extends BaseArenaUserCommand {
			...
			switch (strtolower($args[0])) {
				...
				case "invite":
					if (isset($args[1])) {
						...
						if (!$player->hasPartyInvitesEnabled()) return TextFormat::RED . "That player is not accepting party invites at this time!";

						if ($player->inParty()) return TextFormat::RED . "That player is already in a party!";

						if ($sender->inParty() and count($sender->getParty()->getMembers()) >= Party::$MAX_PLAYERS) return TextFormat::RED . "Your party already contains the maximum amount of players!";
	...
	private function sendPartyHelp(CommandSender $sender) {
		$messages = [TextFormat::GRAY . "----- Party Command Help -----", TextFormat::GOLD . "/party accept [player]" . TextFormat::GRAY . " - Accept party invites", TextFormat::GOLD . "/party chat [msg]" . TextFormat::GRAY . " - Send a chat message to your party", TextFormat::GOLD . "/party disband" . TextFormat::GRAY . " - Disband a party if in one(Leader only)", TextFormat::GOLD . "/party deny" . TextFormat::GRAY . " - Deny party invites!", TextFormat::GOLD . "/party invite [player]" . TextFormat::GRAY . " - Invite players to parties(Creates a party if not in one)", TextFormat::GOLD . "/party kick [player]" . TextFormat::GRAY . " - Kick members from parties(Leader only)", TextFormat::GOLD . "/party leave" . TextFormat::GRAY . " - Leave a party if in one", TextFormat::GOLD . "/party list" . TextFormat::GRAY . " - Lists all members in a party"];
